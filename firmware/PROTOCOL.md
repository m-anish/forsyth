...
| REG0 | UART **9600 8N1**, air rate **2400 bps** | 9600 is what PROGRAM mode requires; keeping data mode at the same baud means never re-initializing the UART around mode changes (lokki learned this the hard way). 2400 air rate buys sensitivity/range. ⚠️ **"airtime doesn't matter" — superseded 2026-08-09.** The first real discharge measurement makes airtime the *dominant* load on a leaf: at a 5-minute cadence the radio costs ~45 mAh/day at 2.4 k against a whole-station budget of ~50, and each air-rate step roughly halves it (TX current is constant; only its duration changes). Air rate is therefore a **per-site knob**, not a fleet constant — fastest rate that reliably closes the link, since a retry costs a whole extra session. Note the fleet can only run mixed rates if the coordinator carries one radio per rate. See [board-a-rev2-brainstorm.md §4g-bis and §4h](../hardware/boards/board-a-rev2-brainstorm.md). |
